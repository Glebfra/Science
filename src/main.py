...
def network():
    ...
    temperature, pressure, density = saturation_line.data
...
def network_phase():
    ...
    temperature, pressure, density = phase_diagram.data
...
def viral_network():
    ...
    model = tf.keras.models.Model(inputs=inputs, outputs=outputs, name='Network')
...
def saturation():
    ...
    plotter = Plotter(dimension='3d')
    plotter.scatter(*saturation_line.data, color='b')

    new_rho = np.exp(
        saturation_line.mapping(
            (1 / saturation_line.temperature, np.log(saturation_line.pressure)), *saturation_line.line_popt
        )
    )
    print(f'popt: {saturation_line.line_popt}')
    print(f'R: {r_squared(saturation_line.data[2], new_rho)}')
    plotter.plot(*saturation_line.data[0:2], new_rho, 'c-')
...
def phase():
    ...
    phase_data = list(map(np.array, phase_diagram.data))
    saturation_data = list(map(np.array, saturation.data))
    ...
    plotter.scatter(*phase_diagram.data, color='r')
...
def only_phase():
    ...
    plotter = Plotter(dimension='3d')
    plotter.scatter(*phase_diagram.data)
    plotter.plot(*saturation_line.data)
    plotter.xlabel(r'$T$ $(K)$')
    plotter.ylabel(r'$p$ $(Pa)$')
    plotter.zlabel(r'$\rho$ $(kg/m^3)$')
    plotter.show()
